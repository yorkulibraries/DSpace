...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:dri="http://di.tamu.edu/DRI/1.0/" xmlns:mets="http://www.loc.gov/METS/" xmlns:dim="http://www.dspace.org/xmlns/dspace/dim" xmlns:xlink="http://www.w3.org/TR/xlink/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:ore="http://www.openarchives.org/ore/terms/" xmlns:oreatom="http://www.openarchives.org/ore/atom/" xmlns="http://www.w3.org/1999/xhtml" xmlns:xalan="http://xml.apache.org/xalan" xmlns:encoder="xalan://java.net.URLEncoder" xmlns:util="org.dspace.app.xmlui.utils.XSLUtils" exclude-result-prefixes="xalan encoder i18n dri mets dim  xlink xsl">
	...
	<xsl:template name="itemSummaryView-DIM-fields">
		...
		<xsl:choose>
			...
			<!-- *YDJ* Description Local row -->
			<xsl:when test="$clause = 4 and (dim:field[@element='description' and @qualifier='local'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-description-local</i18n:text>:</td>
					<td class="cell">
						<xsl:for-each select="dim:field[@element='description' and @qualifier='local']">
							<xsl:copy-of select="./node()"/>							
			...
			<!-- *YDJ* Type row -->
			<xsl:when test="$clause = 6 and (dim:field[@element='type' and not(@qualifier)])">
				<tr>
					<td class="label-cell">
			...
			<!-- *YDJ* Latitude row -->
			<xsl:when test="$clause = 7 and (dim:field[@element='coverage' and @qualifier='y'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-coverage-y</i18n:text>:</td>
					<td class="cell">
						<xsl:for-each select="dim:field[@element='coverage' and @qualifier='y']">
							<xsl:copy-of select="./node()"/>
							<xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='y']) != 0">
	                    	;
	                    </xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* Longitude row -->
			<xsl:when test="$clause = 8 and (dim:field[@element='coverage' and @qualifier='x'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-coverage-x</i18n:text>:</td>
					<td class="cell">
						<xsl:for-each select="dim:field[@element='coverage' and @qualifier='x']">
							<xsl:copy-of select="./node()"/>
							<xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='x']) != 0">
	                    	;
	                    </xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* Municipality row -->
			<xsl:when test="$clause = 9 and (dim:field[@element='coverage' and @qualifier='city'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-coverage-city</i18n:text>:</td>
					<td class="cell">						
						<xsl:for-each select="dim:field[@element='coverage' and @qualifier='city']">
							<a>
								<xsl:attribute name="href">
								      <xsl:value-of select="concat($context-path,'/browse?type=city&#38;value=')"/>
								      <xsl:copy-of select="./node()"/>
								</xsl:attribute>
								<xsl:copy-of select="./node()"/>
							</a>
							<xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='city']) != 0">
								;
							</xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* Regional Municipality row -->
			<xsl:when test="$clause = 10 and (dim:field[@element='coverage' and @qualifier='county'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-coverage-county</i18n:text>:</td>
					<td class="cell">						
						<xsl:for-each select="dim:field[@element='coverage' and @qualifier='county']">
							<a>
								<xsl:attribute name="href">
								      <xsl:value-of select="concat($context-path,'/browse?type=county&#38;value=')"/>
								      <xsl:copy-of select="./node()"/>
								</xsl:attribute>
								<xsl:copy-of select="./node()"/>
							</a>
							<xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='county']) != 0">
								;
							</xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* Conservation Authority row -->
			<xsl:when test="$clause = 11 and (dim:field[@element='coverage' and @qualifier='region'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-coverage-conserva</i18n:text>:</td>
					<td class="cell">						
						<xsl:for-each select="dim:field[@element='coverage' and @qualifier='region']">
							<a>
								<xsl:attribute name="href">
								      <xsl:value-of select="concat($context-path,'/browse?type=region&#38;value=')"/>
								      <xsl:copy-of select="./node()"/>
								</xsl:attribute>
								<xsl:copy-of select="./node()"/>
							</a>
							<xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='region']) != 0">
								;
							</xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* Right(s) row -->
			<xsl:when test="$clause = 12 and (dim:field[@element='rights' and not(@qualifier)] or dim:field[@element='rights' and @qualifier='article'] or dim:field[@element='rights' and @qualifier='journal'] or dim:field[@element='rights' and @qualifier='publisher'] or dim:field[@element='rights' and @qualifier='uri'])">
				<tr>
					<td class="label-cell">
			...
			<!-- *YDJ* Publisher row -->
			<!--<xsl:when test="$clause = 15 and (dim:field[@element='publisher' and not(@qualifier)])">
				<tr>
					<td class="label-cell">
			...
			<!-- *YDJ* series row -->
			<xsl:when test="$clause = 14 and (dim:field[@element='relation' and @qualifier='ispartofseries'])">
				<tr>
					<td class="label-cell">						
			...
			<!-- *YDJ* format row 
			<xsl:when test="$clause = 16 and (dim:field[@element='format' and not(@qualifier)])">
				<tr>
					<td class="label-cell">						
							<i18n:text>xmlui.dri2xhtml.METS-1.0.item-format</i18n:text>:
					</td>
					<td class="cell">
						<xsl:for-each select="dim:field[@element='format' and not(@qualifier)]">
							<xsl:copy-of select="./node()"/>
							<xsl:if test="count(following-sibling::dim:field[@element='format' and not(@qualifier)]) != 0">
	                    	;
	                    </xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* relation row -->
			<xsl:when test="$clause = 16 and (dim:field[@element='relation' and not(@qualifier)])">
				
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-relation</i18n:text>:
					</td>
					<td class="cell">
			...
			<!-- *YDJ* relation.isformatof row -->
			<xsl:when test="$clause = 17 and (dim:field[@element='relation' and @qualifier='isformatof'])">				
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-relation-isformatof</i18n:text>:
					</td>
					<td class="cell">
					<xsl:variable name="url_flag" select="./dim:field[@element='relation' and @qualifier='isformatof']"/>
						<xsl:if test="string($url_flag) != ''">
							<xsl:choose>
								<xsl:when test="substring($url_flag,1,3) = 'ASC'">
									<a> <xsl:attribute name="href"><xsl:value-of select="concat($context-path,'/discover?order=DESC&amp;sort_by=score&amp;query=')"/>							<xsl:copy-of select="$url_flag"/></xsl:attribute> 
								<xsl:copy-of select="$url_flag"/>
							</a>									
								</xsl:when>
								<xsl:otherwise>
									<a> <xsl:attribute name="href"><xsl:value-of select="concat($context-path,'/discover?order=DESC&amp;sort_by=score&amp;query=')"/>							<xsl:copy-of select="substring($url_flag,10)"/></xsl:attribute> 
								<xsl:copy-of select="$url_flag"/>
							</a>									
								</xsl:otherwise>
							</xsl:choose>
							...
							<xsl:if test="count(following-sibling::dim:field[@element='relation' and @qualifier='isformatof']) != 0">
	                    	;
	                        </xsl:if>
			...
			<!-- relation.mapsuri row -->
			<xsl:when test="$clause = 18 and (dim:field[@element='relation' and @qualifier='mapsuri'])">
				<tr>
					<td class="label-cell">
						<i18n:text>xmlui.dri2xhtml.METS-1.0.item-relation-mapsuri</i18n:text>:</td>
					<td class="cell">
						<xsl:for-each select="dim:field[@element='relation' and @qualifier='mapsuri']">
							<a>
								<xsl:attribute name="href"><xsl:copy-of select="./node()"/></xsl:attribute>
								<xsl:copy-of select="./node()"/>
							</a>
							<xsl:if test="count(following-sibling::dim:field[@element='relation' and @qualifier='mapsuri']) != 0">
								<br/>
							</xsl:if>
						</xsl:for-each>
					</td>
			...
			<xsl:when test="$clause = 20 and $ds_item_view_toggle_url != ''">
