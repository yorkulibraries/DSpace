...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:dri="http://di.tamu.edu/DRI/1.0/" xmlns:mets="http://www.loc.gov/METS/" xmlns:dim="http://www.dspace.org/xmlns/dspace/dim" xmlns:xlink="http://www.w3.org/TR/xlink/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:ore="http://www.openarchives.org/ore/terms/" xmlns:oreatom="http://www.openarchives.org/ore/atom/" xmlns="http://www.w3.org/1999/xhtml" xmlns:xalan="http://xml.apache.org/xalan" xmlns:encoder="xalan://java.net.URLEncoder" xmlns:util="org.dspace.app.xmlui.utils.XSLUtils" exclude-result-prefixes="xalan encoder i18n dri mets dim  xlink xsl">
	...
	<xsl:template name="itemSummaryView-DIM-fields">
		...
		<xsl:choose>
			...
			<!-- *YDJ* Keywords row -->
			<xsl:when test="$clause = 6 and (dim:field[@element='subject' and @qualifier='keywords'])">
				<tr>
					<td class="label-cell">
						<!--<i18n:text>xmlui.dri2xhtml.METS-1.0.item-subject</i18n:text>:</td>-->
						<xsl:text>Keywords</xsl:text>:</td>
					<td class="cell">
						<xsl:for-each select="dim:field[@element='subject' and @qualifier='keywords']">
							<xsl:copy-of select="./node()"/>
							<xsl:if test="count(following-sibling::dim:field[@element='subject' and @qualifier='keywords']) != 0">
								<br/>
							</xsl:if>
						</xsl:for-each>
					</td>
			...
			<!-- *YDJ* supervisor row -->
			<xsl:when test="$clause = 11 and (dim:field[@element='contributor' and @qualifier='advisor'])">
				<tr>
					<td class="label-cell">						
							<i18n:text>xmlui.dri2xhtml.METS-1.0.item-supervisor</i18n:text>:
					</td>
					<td class="cell">
			...
			<!-- *YDJ* degree row -->
			<xsl:when test="$clause = 12 and (dim:field[@element='degree'])">
				<tr>
					<td class="label-cell">						
					...
					<td class="cell">
						...
						<xsl:for-each select="dim:field[@element='degree' and @qualifier='level']">
							<xsl:copy-of select="./node()"/>							
							<xsl:if test="count(following-sibling::dim:field[@element='degree' and @qualifier='level']) != 0">
	                    	;
	                    </xsl:if>
						</xsl:for-each> -->
					</td>
			...
			<!-- *YDJ* program row -->
			<xsl:when test="$clause = 13 and (dim:field[@element='degree' and @qualifier='discipline'])">
				<tr>
					<td class="label-cell">						
							<i18n:text>xmlui.dri2xhtml.METS-1.0.item-program</i18n:text>:
					</td>
					<td class="cell">
			...
			<!-- *YDJ* faculty row 
			<xsl:when test="$clause = 14 and (dim:field[@element='degree' and @qualifier='faculty'])">
				<tr>
					<td class="label-cell">						
					...
					<td class="cell">
						<xsl:for-each select="dim:field[@element='degree' and @qualifier='faculty']">
							<a>
								<xsl:attribute name="href"><xsl:value-of select="concat($context-path,'/browse?type=faculty&#38;value=')"/><xsl:copy-of select="./node()"/></xsl:attribute>
								<xsl:copy-of select="./node()"/>
							</a>							
							<xsl:if test="count(following-sibling::dim:field[@element='degree' and @qualifier='faculty']) != 0">
								<br/>
							</xsl:if>
						</xsl:for-each>						
					</td>
			...
			<!-- date.copyright row -->
			<xsl:when test="$clause = 14 and (dim:field[@element='date' and @qualifier='copyright'])">
				<tr>
					<td class="label-cell">
						<xsl:text>Exam date</xsl:text>:
					</td>
					<td class="cell">
						<xsl:variable name="url_flag" select="./dim:field[@element='date' and @qualifier='copyright']"/>
	...
	<xsl:template match="mets:file">
		...
		<!--YDJ Streaming Code -->
		<xsl:if test="@MIMETYPE='video/mp4' or @MIMETYPE='video/webm'">             
			<xsl:variable name="streamfile" select="mets:FLocat/@xlink:href" />	
			<xsl:variable name="id" select="substring-before(mets:FLocat[@LOCTYPE='URL']/@xlink:title, '.')"/>			
			<video id="{$id}" class="video-js vjs-default-skin vjs-big-play-centered" controls="controls" preload="none" width="640" height="320"
				   poster="/dspace/themes/ETD/lib/Desert.jpg" data-setup="">
				<source src="{$streamfile}" type="video/mp4" />				
				<p class="vjs-no-js">To view this video please enable JavaScript, and consider upgrading to a web browser that <a href="http://videojs.com/html5-video-support/" target="_blank">supports HTML5 video</a></p>
			</video>			
		</xsl:if>
		<xsl:if test="@MIMETYPE='audio/mpeg' or @MIMETYPE='audio/wav'">             
			<xsl:variable name="streamfile" select="mets:FLocat/@xlink:href" />	
			<xsl:variable name="id" select="substring-before(mets:FLocat[@LOCTYPE='URL']/@xlink:title, '.')"/>			
			<audio id="{$id}" controls="controls" preload="none" width="640" height="320">
				<source src="{$streamfile}" type="audio/mpeg" />				
				<embed src="{$streamfile}" />				
			</audio>			
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>
